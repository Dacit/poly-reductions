...
HOL_To_IMP_correct HOL_To_HOL_Nat.list_card_nat by cook
...
HOL_To_IMP_correct HOL_To_HOL_Nat.is_vc_list_nat by (cook mode = induction)

lemma is_vc_list_IMP_correct:
  assumes "Rel_nat (s ''is_vc_list_nat.arg.x'') x"
  shows "terminates_with_res_IMP (tailcall_to_IMP is_vc_list_nat_IMP_tailcall) s
...
proof -
  ...
  then show ?thesis using is_vc_list_nat_IMP_correct[of s, OF assms] by simp
qed
